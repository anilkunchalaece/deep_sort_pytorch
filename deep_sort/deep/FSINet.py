...
class FusedSimilarityNet(nn.Module):
    def __init__(self,config):
        ...
        if config["FUSED"] == True :
            ...
            self.img_attn = nn.MultiheadAttention(config["EMBED_FC2_OUT"],8,batch_first=True)
            self.embed_attn = nn.MultiheadAttention(config["EMBED_FC2_OUT"],8,batch_first=True)
    ...
    def forward_one(self,x):
        ...
        if self.config["FUSED"] == True :    
            ...
            if embed_base.dim() == 1 :
                embed_base = embed_base.unsqueeze(0)
            ...
            #     "embed_f" : embed_f
            # }
            # print("sending fused features")
            out = out.div(out.norm(p=2,dim=1,keepdim=True))
            return out
        else :
            # print("sending only img features")
            img_f = img_f.div(img_f.norm(p=2,dim=1,keepdim=True))
            return img_f
